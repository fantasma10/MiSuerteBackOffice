...
$headerStyle = 'style = color:#124a7e;font-weight:bold; font-size:12px;';
	...
	*/
		if(!empty($arrTable["mainHead"])){
			...
			$tbl.= "<tr>";
				$tbl.= "<td style =\"text-align:center;color:#35659d;text-decoration: none;font-size:30px;font-face: Arial,Helvetica;\" colspan=\"3\">".utf8_encode($arrTable["mainHead"])."</td>";
			$tbl.= "</tr>";
		...
		if(!empty($arrTable["filtersHead"]) && !empty($arrTable["filtersValue"])){
			...
			foreach($filtersHead AS $fh){
				...
				$tbl.= "<td align=\"left\" style =\"text-align:left;font-size:15px;font-face:Arial Helvetica;\"><span style=\"font-weight:bold;\">".$fh."</span> ".$dospuntos." ".$filtersValue[$c]."</td>";
	...
	*/
	...
	if(!empty($arrTable["headers"])){
		...
		foreach($headers AS $header){
			...
			//$tbl.= "<th width=\"$widths[$contw]\" style =\"padding-bottom:60px;\">".utf8_decode($header)."</th>";
			$contw++;
		}
		$tbl.= "</tr>";
		/*$tbl.="<tr style='height:0px;'>
			<td colspan='7'><hr></td>
		</tr>";*/
	}
			...
			*/
			if(!empty($indexes)){
				...
				foreach($indexes AS $index){
					...
					//$tbl.= "<td width=\"$widths[$contw]\" style='padding:10px;' >".$value."</td>";
					$cont++;
					$contw++;
				}
				$cont = 0;
				...
				foreach($row AS $cell){
					...
					//$tbl.= "<td width=\"$widths[$contw]\" style='padding:10px;' class=\"$class\">".$cell."</td>";
					$cont++;
				}
				$cont = 0;
			}
