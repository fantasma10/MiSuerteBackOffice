...
function initView(){
    ...
    var mesI = (date.getMonth()+1) < 10 ? '0'+(date.getMonth()+1) : (date.getMonth()+1);
    ...
    $('#dFechaInicio').val(fechaInicio);
		...
		_llenarTabla : function(){
				...
				var fecha = $('#dFechaInicio').val();
                                if(fecha != ""){
                                    
                                    $.post(BASE_URL +"/ayddo/ajax/ReportePlandePago.php",
                                    {
...
//							'<td >'+obj[index]['nIdProveedor']+'</td>'+
                                                        ...
                                                        '<td >'+obj.Data[index]['sCLABE']+'</td>'+
                                                        '<td >'+obj.Data[index]['nSaldo']+'</td>'+
                                                        '<td > '+obj.Data[index]['nMonto']+'</td>'+
                                                        '<td > '+obj.Data[index]['nCargo']+'</td>'+
                                                        '<td > '+obj.Data[index]['nTotal']+'</td>'+
//							'<td > <button id="verDetalle" data-proveedor='+obj[index]['nIdProveedor']+' data-nombreProveedor ="'+obj[index]['sNombre']+'";  data-placement="top" rel="tooltip" title="Ver Detalle" class="btn habilitar btn-default btn-xs" data-title="Ver Informacion"><span class="fa fa-search"></span></button>'+
                                            '</tr>');
                                              ...
                                              if (obj.Data.length <= 0) {
                                                ...
                                                $('form[name=excel] :input[name=h_fechaSolicitud]').val(fechaInicio);
                                        $('form[name=pdf] :input[name=h_fechaSolicitud]').val(fechaInicio);
                                        }else{
...
//                            $("#data tbody").empty();
    ...
    //							'<td > <button id="verDetalle" data-proveedor='+obj[index]['nIdProveedor']+' data-nombreProveedor ="'+obj[index]['sNombre']+'";  data-placement="top" rel="tooltip" title="Ver Detalle" class="btn habilitar btn-default btn-xs" data-title="Ver Informacion"><span class="fa fa-search"></span></button>'+
                                        ...
                                        $('form[name=excel] :input[name=h_fechaSolicitud]').val($('#dFechaInicio').val());
                                        $('form[name=pdf] :input[name=h_fechaSolicitud]').val($('#dFechaInicio').val());
                                    })
                                    .fail(function(resp){
                                                    alert('Ha ocurrido un error, intente de nuevo m\u00E1s tarde');
                                    });
	      ...
	      if(conteo > 0){
	          ...
	          // Llenado de la tabla con la informacion de la consulta
	           jQuery.each(obj, function(index,value) {
	              ...
	              '<td style="text-align:center;">'+obj[index]['nIdLineaNegocio']+'</td>'+
...
function mostrarActivacionCuenta(nIdEstatus,nIdProveedor){
	$('#activaCuenta').empty();
	accionCuenta = nIdEstatus != 0 ? 'Activar Cuenta <i class="fa fa-check" aria-hidden="true"></i>' : 'Desactivar cuenta <i class="fa fa-times" aria-hidden="true"></i>'; 
	
	$('#activaCuenta').append('<a href ="#" onclick ="cambiarStatus('+nIdEstatus+','+nIdProveedor+')">'+accionCuenta+'</a>');
 ...
 function cargarColonias(sCodigoPostal){
	...
	$('#empresaColonia').on('storeLoaded', function(e, resp){
		if(resp.bExito){
			...
			if(resp.data.length == 1){
				$('#empresaColonia').val(resp.data[0].nIdColonia);
			}
		}
	});
}
