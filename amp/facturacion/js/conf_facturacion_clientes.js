...
function validacion(){
    ...
    if( $("#txtSerie").val() !=''   && 
        ...
        if( $("#txtSerie").val() !=''   || 
            ...
            jAlert('Favor de llenar todos los campos de la configuracion de Tiempo Aire' , 'Mensaje');
            return false;
        }
    }
    ...
    if( $("#txtSerieS").val() !=''   &&  
        ...
        if( $("#txtSerieS").val() !=''   ||  
            ...
            $("#cmbProductoS").val() !='' || 
            $("#cmbIVAS").val() !=''){
            jAlert('Favor de llenar todos los campos de la configuracion Servicios' , 'Mensaje');
            return false;
        }
    }
 ...
 function getCatalogos(){
    return new Promise(resolve =>{
        ...
        ).fail(function(resp){jAlert('Ha ocurrido un error, intente de nuevo m\u00E1s tarde', 'Mensaje'); resolve(false);});
    });
    ...
    switch (validacion) {
        ...
        case 2://TAE
            data=[{
                ...
                "scmbMetodoPago":$( "#cmbMetodoPago option:selected" ).text(),
                "cmbFormaPago":$("#cmbFormaPago").val(),
                "scmbFormaPago":$( "#cmbFormaPago option:selected" ).text(),
                "cmbCFDI":$("#cmbCFDI").val(),
                "scmbCFDI":$( "#cmbCFDI option:selected" ).text(),
                "cmbProducto":$("#cmbProducto").val(),
                "scmbProducto":$( "#cmbProducto option:selected" ).text(),
        ...
        default://NINGUNO DE LOS 2
         // jAlert('Favor de llenar los campos.');
          return false;
        break;  
    }
     
    
    $.post("../ajax/guardar_conf_facturacion_cliente.php",{data},
    function(response){
        ...
        switch (validacion) {
            case 1:
                ...
                jAlert('Se han agregado correctamente los datos', 'Mensaje');
            break;
            case 2:
                $("#idFacturacionTAE").val(data.data.TAE);
                jAlert('Se han agregado correctamente los datos', 'Mensaje');
...
async function getConfiguracionClientes(){
    showSpinner();
var resultado= await getCatalogos();
    if(resultado){
        $.post("../ajax/conf_getConfiguracion_cliente.php",{id:0},
            function(response){
                hideSpinner();
                var data =JSON.parse(response);
        ...
        ).fail(function(resp){jAlert('Ha ocurrido un error, intente de nuevo m\u00E1s tarde', 'Mensaje');})
    }
}