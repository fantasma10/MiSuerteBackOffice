...
function buscarColonias(nCodigoPostal, nIdColonias){
	...
	.done(function(resp){
		if(resp.bExito) {
            ...
            if(nIdColonias != undefined){
                $('#cmbColonia').val(nIdColonias);
            	$('#cmbColonia').prop('disabled', true);
            }

			if (DATA_APARTADO_DIR) {
				$('#cmbColonia').val(DATA_APARTADO_DIR.idcColonia);
				$('#cmbCiudad').val(DATA_APARTADO_DIR.idcMunicipio);
				$('#cmbEntidad').val(DATA_APARTADO_DIR.idcEntidad);
			}
		}
	})
