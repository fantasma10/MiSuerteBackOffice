...
function initInstruccionPagoProveedor(){
	...
	cargarStore(BASE_PATH + "/inc/Ajax/stores/storeAcreedores.php", "cmbAcreedor", {}, {value : 'idProveedor', text : 'nombreProveedor'}, {}, '');
...
function BuscarInstruccionesProveedor(){
	...
	$('#gridbox').html('<table id="tblGridBox" class="display table table-bordered table-striped"><thead><tr><th>Id Inst.</th><th>Proveedor</th><th>Cuenta</th><th>Fecha</th><th>Importe</th><th>IVA</th><th>Total</th><th>Descripción</th><th>Estatus</th><th>Acciones</th></tr></thead><tbody></tbody></table>');
	$('#botones_excel').show();
	$('#gridbox').show();

	var parametros = getParams($("form").serialize());

	llenarDataTable('tblGridBox', parametros, BASE_PATH + "/inc/Ajax/_Contabilidad/InstruccionesProveedor/BuscarInstruccionesAcreedor.php", customsettings);
}

function resetInstruccion(idInstruccion){
	if(confirm("\u00BFDesea Reestablecer la Instrucción\u003F")){
		$.post(BASE_PATH + '/inc/Ajax/_Contabilidad/InstruccionesProveedor/reestablecerInstruccionAcreedor.php',
			{
