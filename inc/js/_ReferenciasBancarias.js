...
function BuscarReferenciasBancarias(){
	...
	if ( $("#tblGridBox").length ) {
		$("#tblGridBox").dataTable().fnDestroy();
		$("#tblGridBox").remove();
	}
	
	var cadena = document.getElementById("idCadena").value;
	var subcadena = document.getElementById("idSubCadena").value;
	var corresponsal = document.getElementById("idCorresponsal").value;
	var parametros = "";
	...
	$('#botones_excel').show();
	var tabla = '<table id="tblGridBox" class="display table table-bordered table-striped"><thead><th>Cadena</th><th>SubCadena</th><th>Cuenta bancaria</th><th>Referencia</th></thead><tbody><td></td><td></td><td></td><td></td></tbody></table>';
	...
	llenaDataTableR('tblGridBox', params, BASE_PATH + '/inc/Ajax/_Contabilidad/BuscaReferencias.php');
...
function llenaDataTableR(id, obj, url){
...
function llenaDataTableRef(id, obj, url){
...
function ExportarRefBancariasMiRed(){
	

	document.getElementById("formaBanco").submit();

	
}

function showExcelCommon(url){
