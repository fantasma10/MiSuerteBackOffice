...
function seleccionarPantalla(event){
	...
	if(valorSeleccioando == 1){// Cargar la pantalla de Consulta
		cargarContenidoHtml(BASE_PATH + '/_Contabilidad/FacturasRecibos/pantallaConsultaGrupos.php', 'htmlContent', 'initConsultaClientes()');
	}
	if(valorSeleccioando == 2){
		cargarContenidoHtml(BASE_PATH + '/_Contabilidad/FacturasRecibos/pantallaCrearGrupos.php', 'htmlContent', 'initFacturasClientes()');
	}
}
...
function validarinfo(){
	...
	if ( $("#txtNumFactura").val() != "" || $("#txtUUID").val() != "" ) {
					...
					success		: function(response, textStatus, jqXHR){
						...
						if(params.idFactura == 0){
							if(items[0] == 0){
								...
								$('#formRedireccion [name="idCadena"]').val(params.idCadena);
...
function BuscarFacturas(){
	...
	llenaDataTable("tblGridBox", params, BASE_PATH + "/inc/Ajax/_Contabilidad/BuscaFacturasGrupos.php");
}

function editar(id){
	cargarContenidoHtml(BASE_PATH + '/_Contabilidad/FacturasRecibos/pantallaCrearGrupos.php', 'divTbl', 'initFacturasClientesEditar();loadDocumento(' + id + ')');
}
...
function AbrirDetalleFactura(tipoDocumento, numeroCuenta, noFactura){
	...
	var url = BASE_PATH + "/_Contabilidad/FacturasRecibos/ConsultaGrupo.php?idFactura="+idFactura;
	cargarContenidoHtml(url, 'divTbl', "loadDocumento("+ idFactura +")");
}

function loadDocumento(idDoc){
	$.post(BASE_PATH + '/inc/Ajax/_Contabilidad/loadFacturaGrupo.php',
		{
			idDocumento : idDoc
		},
		function(response){
...
function showExcel(todo){
	...
	$.fileDownload(BASE_PATH + "/inc/Ajax/_Contabilidad/ListaExcelFacturasGrupos.php?" + params, {
