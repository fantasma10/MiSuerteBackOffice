<?php
	...
	if($destino == 1){
		...
		$sql = $WBD->query("CALL `data_contable`.`SP_ALTA_CUENTA_CONF`($idUsuario, $instruccion, $tipoMovimiento, 0, '$numCuenta', @x, @y)");
	...
	if($destino == 2){
		...
		$sql = $WBD->query("CALL `data_contable`.`SP_ALTA_CUENTA_BANCO`(0, $instruccion, $tipoMovimiento, '$numCuenta',	'$clabe', '$rfc', '', '$beneficiario', '$correo', @x, @y, '$idUsuario');");
