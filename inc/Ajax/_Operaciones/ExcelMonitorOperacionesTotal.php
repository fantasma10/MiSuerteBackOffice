<?php
	...
	if ( !empty($idCadena) && !empty($fechaInicial) && !empty($fechaFinal) ) {
		...
		O.`referencia1Operacion`,O.`respuestaOperacion`,P.`nombreProveedor`,O.`autorizacionOperacion`,O.`idsOperacion`
		...
		INNER JOIN `redefectiva`.`dat_proveedor` as P on P.`idProveedor` = O.`idProveedor` AND P.`idEstatusProveedor` = 0
		...
		if($RBD->error() == ''){
			...
			if($res != '' && mysqli_num_rows($res) > 0){
				...
				$d .= "</thead>";
				$d .= "<tbody>";
				while( list($idcad,$idcorr,$idemisor,$idtrantype,$fecaplicacion,$importe,$referencia,$respuesta,$nomProveedor,$autorizacion,$idsOperacion) = mysqli_fetch_array($res)){
					...
					$d .= "<td>".htmlentities($autorizacion)."</td>";
					$d .= "<td>".htmlentities($idsOperacion)."</td>";
					$d .= "</tr>";
				}			
				$d .= "</tbody>";
				$d .= "</table>";
