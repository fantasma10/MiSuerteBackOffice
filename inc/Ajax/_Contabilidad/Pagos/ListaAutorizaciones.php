<?php
	...
	$sql = "CALL data_contable.SP_AUTORIZACION_LOAD3($tipoPago, '$noFactura', '$fechaFactura', '$RFC', '$numeroCuenta', $idEstatus, $actual, $cant, $colsort, '$ascdesc', '$strToFind')";
	//var_dump("sql: $sql");
	...
	while($row= mysqli_fetch_assoc($result)){
		...
		if($esEscritura){
			if(in_array($idUsuario, $autorizadores)){
				...
				$idDocumento = ($row['idTipoCortePago'] == 5)? $row['idReembolso'] : $row['iddocumento'];
				if(($idPerfil == 3 || $idPerfil == 1) && $row['idEstatus'] == 1){
					$autorizar = "<a href='#ModalEditar' data-toggle='modal' data-keyboard='false' idtipocortepago='".$row['idTipoCortePago']."' idautorizacion='".$row['idAutorizacion']."' idDocumento='".$idDocumento."' tipocorte='".$row['idTipoCortePago']."' onclick='abrirDetalle(event);'>
						<img src='../../../img/ico_revision2.png' data-original-title='Autorizar' idtipocortepago='".$row['idTipoCortePago']."' idautorizacion='".$row['idAutorizacion']."' idDocumento='".$idDocumento."' tipocorte='".$row['idTipoCortePago']."' numCuenta='".$row['numeroCuenta']."' noFactura='".$row['noFactura']."' tipoDocumento='".$row['tipoDocumento']."' origenDocumento='".$row['origenDocumento']."' nombreDocumento='".$row['nombreDocumento']."' idusuarioautorizador='".$idUsuarioAutorizador."'>
					</a>";
				}else{
					$autorizar = "";
				}
		...
		$numerosFactura = "";
		if ( $row['totalFacturasNA'] > 0 ) {
			if ( !empty($row["facturas"]) ) {
				$numerosFactura = acentos($row["facturas"]).", N/A (".$row['totalFacturasNA'].")";
			} else {
				$numerosFactura = "N/A (".$row['totalFacturasNA'].")";
			}
		} else {
			$numerosFactura = acentos($row["facturas"]);
		}
