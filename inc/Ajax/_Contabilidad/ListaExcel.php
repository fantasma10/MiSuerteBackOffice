<?php
	...
	*/
	...
	if($idTipoProveedor == 0){
		$sql = "CALL data_contable.SP_FACTURAS_LOAD($tipoDocumento,'$noFactura','$numeroCuenta','$fechaFactura', '$fechaInicio','$fechaFin',$idEstatus, $start, $limit, $colsort, '$ascdesc', '$strToFind', $idTipoProveedor, 0, $corte, $idProveedor)";
	}
	else if($idTipoProveedor == 1){
		$sql = "CALL `data_contable`.`SP_FACTURAS_ACREEDOR_LOAD`($tipoDocumento, '$noFactura', '$numeroCuenta', '$fechaFactura', '$fechaInicio', '$fechaFin', $idEstatus, $start, $limit, $colsort, '$ascdesc', '$strToFind', $idTipoProveedor, 0, $corte, $idProveedor)";
	}
