<?php
	...
	*/
	...
	/*
		Crear poliza de ingresos conciliados en dias diferente fecha
	*/
	...
	$oPolizaHeader->setDFecha($dFecha2);
	$oPolizaHeader->setNIdTipoPoliza(1);
	$oPolizaHeader->setSFolio($sFolio2);
	$oPolizaHeader->setNIdClase(1);
	$oPolizaHeader->setNIdDiario(0);
	$oPolizaHeader->setSConcepto($sConcepto2);
	...
	if(count($oPoliza->arrayMovimientos) > 0){
		...
		$nIdPoliza2 = $resultado['data']['nIdPoliza'];
	}
	else{
		liberarBandera();
		echo json_encode(array(
			'nCodigo'	=> 1,
			'bExito'	=> false,
			'sMensaje'	=> 'No se encontraron movimientos'
		));
		exit();
	}

	$resultado['data'] = array(
		'nIdPoliza2'	=> $nIdPoliza2
	);

	if($nIdPoliza2 == 0){
		$resultado['sMensaje'] = "No fue posible generar la Póliza";
	}
	if($nIdPoliza2 > 0){
		$resultado['sMensaje'] = "Poliza de Ingresos Complementaria Generada Correctamente";
	}
