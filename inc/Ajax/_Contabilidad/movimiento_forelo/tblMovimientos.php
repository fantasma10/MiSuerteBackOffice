<?php
	...
	$nIdTipoMovimiento	= !empty($_POST['nIdTipoMovimiento'])							? $_POST['nIdTipoMovimiento']	: 0;
	...
	$resultado = $oMovimiento->consulta($array_params);
	...
	if($resultado['bExito'] == false || $resultado['nCodigo'] != 0 || $resultado['num_rows'] == 0){
		...
		foreach($aaData as $key => $row){
			$aaData[$key]['cargoMov']	= "\$ ".number_format($row['cargoMov'], 2, '.', ',');
			$aaData[$key]['abonoMov']	= "\$ ".number_format($row['abonoMov'], 2, '.', ',');
			$aaData[$key]['saldoFinal'] = "\$ ".number_format($row['saldoFinal'], 2, '.', ',');
		}
