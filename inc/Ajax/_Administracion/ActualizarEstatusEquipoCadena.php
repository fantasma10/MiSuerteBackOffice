<?php
	...
	$idC = (isset($_POST['idCadena']))?$_POST['idCadena']: -2;
	...
	if ($idC != -1){	
		$res = $WBD->query("CALL `nautilus`.`SP_EQUIPO_ACTIVARCADENA`($idEquipo, $idC)");	
		var_dump("CALL `nautilus`.`SP_EQUIPO_ACTIVARCADENA`($idEquipo, $idC)");	
		if($WBD->error() == ''){
			$output = array(
				'codigo' => 0,
