...
function conciliaManual(){
	...
	if(lengthBanco != 1){
		jAlert('Seleccione un solo Movimiento Bancario');
		return false;
	}

	if(lengthDeposito != 1){
		jAlert('Seleccione un solo Dep\u00F3sito');
		return false;
	}
...
function desconciliar(){
	...
	if(lengthBanco <= 0){
		jAlert('Seleccione un Movimiento Bancario');
		return false;
	}
	...
	if(nIdEstatusBanco != 2){
		jAlert('Solo es posible desconciliar los movimientos con estatus "Conciliaci\u00F3n Parcial"');
		return false;
	}
