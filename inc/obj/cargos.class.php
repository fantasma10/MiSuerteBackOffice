<?php
	...
	class Cargo{
		...
		public function __construct($LOG, $CONN, $CONN2, $idConf = null, $idConcepto = null, $idCadena=0, $idSubCadena=0, $idCorresponsal=0, $importe = 0, $fechaInicio="", $Observaciones="", $descripcion="", $idEstatus = 0, $numCuenta = 0, $tipoCargo = 0){
			$this->CONN = $CONN;
			$this->CONN2 = $CONN2;
		...
		*/
		public function cargarTodos(){
			if($this->CONN2 != null){
				$query = "CALL redefectiva.SP_LOAD_CARGOS(".$this->idCadena.", ".$this->idSubCadena.", ".$this->idCorresponsal.")";
		...
		public function cargarCuotas(){
			if($this->CONN2 != null){
				$query = "CALL redefectiva.SP_GET_CUOTAS(".$this->idCadena.", ".$this->idSubCadena.", ".$this->idCorresponsal.")";
				$sql = $this->CONN2->query($query);
				if(!$this->CONN2->error()){
		...
		public function cargarAfiliaciones(){
			if($this->CONN2 != null){
				$query = "CALL redefectiva.SP_GET_AFILIACIONES(".$this->idCadena.", ".$this->idSubCadena.", ".$this->idCorresponsal.")";
				$sql = $this->CONN2->query($query);
		...
		public function crearCargoPorCorresponsal(){
			if($this->CONN2){
				...
				if(!$this->CONN2->error()){
					...
					$result["error_msg"] = $this->CONN2->error();
				}
			}//if ! $this->CONN2
			else{
				$result["error"] = 1;
		...
		*/
		public function crearCargo(){
			if($this->CONN != null){
				...
				$query = "CALL redefectiva.SP_CREATE_CARGO(".$this->idConcepto.", ".$this->idCadena.", ".$this->idSubCadena.", ".$this->idCorresponsal.", ".$this->importe.", '".$this->fechaInicio."',
															'".$this->Observaciones."', '".$this->descripcion."', ".$this->idEstatus.", '".$this->numCuenta."', ".$this->tipoCargo.", ".$this->idUsuario.");";

				$sql = $this->CONN->query($query);
				//echo "<pre>"; echo var_dump($this->CONN->error()); echo "</pre>";
				$row = mysqli_fetch_assoc($sql);
				...
				if(!$this->CONN->error()){
					...
					$row["errmsg"] = "Ha ocurrido un error";
				}
			}
			else{
				$res["error"] = 1;
		...
		*/
		public function actualizarCargo(){
			if($this->CONN != null){
				$sql = $this->CONN->query("CALL redefectiva.SP_UPDATE_CARGO(".$this->idConf.", ".$this->idConcepto.", ".$this->importe.", '".$this->fechaInicio."', '".$this->Observaciones."')");
		...
		*/
		public function deleteCargo(){
			$query = "CALL redefectiva.SP_DELETE_CARGO(".$this->idConf.", ".$this->idEstatus.", ".$this->tipoCargo.")";
			if($this->CONN != null){
				$sql = $this->CONN->query($query);
