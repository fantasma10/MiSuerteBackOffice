<?php
	...
	if ( $codigoPostal > 0 ) {
		...
		$sql = "CALL `pronosticos`.`sp_select_colonia`('$codigoPostal');";
		$listaColonias = array();
		$result = $MRDB->SP($sql);
		if ( $MRDB->error() == '' ) {
			...
			echo json_encode( array( "codigoDeRespuesta" => 2, "mensajeDeRespuesta" => "No se pudo consultar la base de datos: ".$MRDB->error() ) );
		}
