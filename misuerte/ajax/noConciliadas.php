...
$filename = (!empty($arrTable["mainHead"]))? str_replace(" ", "_", $arrTable["mainHead"]) : "Conciliacion";

header("Content-type=application/x-msdownload");
header("Content-disposition:attachment;filename=Conciliacion_".$fecha.".xls");
...
*/
	if(!empty($arrTable["mainHead"])){
		...
		$tbl.= "<tr>";
		$tbl.= "<td style =\"padding-bottom:330px;text-align:center;color:#35659d;text-decoration: none; font-size:17px;font-face: Arial,Helvetica;\" colspan=\"3\">".utf8_encode($arrTable["mainHead"])."</td>";
		$tbl.= "</tr>";
	...
	if(!empty($arrTable["filtersHead"]) && !empty($arrTable["filtersValue"])){
		...
		foreach($filtersHead as $fh){
			...
			$tbl.= "<td align=\"left\" class=\"textCell\" style =\"padding:30px;text-align:center;font-size:14px;font-face:Arial Helvetica;\"><span style=\"font-weight:bold;\">".$fh."</span> ".$dospuntos." ".$filtersValue[$c]."</td>";
...
*/
$tbl .= "<table border=\"0\" cellpadding='2' cellspacing='2' align='center' style=\"margin-top:50px;\">";
...
if(!empty($arrTable["headers"])){	
	...
	foreach($headers as $header){
		$tbl.= "<th style =\"padding-bottom:60px;text-align: center;\">".utf8_decode($header)."</th>";
		$contw++;
	}
	$tbl.= "</tr>";
}
		...
		*/
		if(!empty($indexes)){
			...
			foreach($indexes as $index){
				...
				$tbl.= "<td class=\"textCell\" style='padding:10px;' >".$value."</td>";
				$cont++;
				$contw++;
			}
			$cont = 0;
			...
			foreach($row as $cell){
				$tbl.= "<td class=\"textCell\" style='padding:10px;' class=\"$class\">".$cell."</td>";
				$cont++;
			}
			$cont = 0;
		}
