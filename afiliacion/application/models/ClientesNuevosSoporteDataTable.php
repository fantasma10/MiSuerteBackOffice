<?php
	...
	if ( $_GET['sSearch'] != "" ) {
		...
		$sQuery = "CALL `afiliacion`.`SP_CARGAR_CLIENTESNUEVOSOPORTE_DATATABLE`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	} else {
		$sColumns = str_replace(" , ", " ", implode(", ", $aColumns));
		$sQuery = "CALL `afiliacion`.`SP_CARGAR_CLIENTESNUEVOSOPORTE_DATATABLE`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	}
	...
	$sQuery = "CALL `afiliacion`.`SP_CARGAR_CUENTA_CLIENTESNUEVOSOPORTE_DATATABLE`();";
