<?php
	 ...
	 */
		$aColumns = array( 'a.idCliente', 'a.RFC', 'if(a.idRegimen = 1, concat(a.Nombre," ",a.Paterno ," ",a.Materno), a.RazonSocial) ', 'f.sucursales', 'a.FecRegistro', '\'\'', '\'\'' , '\'\'');
	...
	if ( $_GET['sSearch'] != "" ) {
		...
		$sQuery = "CALL `afiliacion`.`SP_SELECT_CLIENTES_EQUIPOS_CONFIGURAR_DT`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	} else {
		$sColumns = str_replace(" , ", " ", implode(", ", $aColumns));
		$sQuery = "CALL `afiliacion`.`SP_SELECT_CLIENTES_EQUIPOS_CONFIGURAR_DT`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	}
	...
	$sQuery = "CALL `afiliacion`.`SP_SELECT_CUENTA_CLIENTES_EQUIPOS_CONFIGURAR_DT`();";
