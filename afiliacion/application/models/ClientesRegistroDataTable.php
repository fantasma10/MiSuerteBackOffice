<?php
	 ...
	 */
	$aColumns = array( 'nIdIncremental', 'P.sRFC', 'P.bCorreoValidado', 'P.sNombreContacto', 'P.sEmail','P.nIdPais', 'P.nIdRegimen','P.nIdEstatus' ,'\'\'', '\'\'' );
	...
	if ( $_GET['sSearch'] != "" ) {
		...
		$sQuery = "CALL `afiliacion`.`SP_CARGAR_PROPECTOMODCERO_DATATABLE`( '$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	} else {
		$sColumns = str_replace(" , ", " ", implode(", ", $aColumns));
		$sQuery = "CALL `afiliacion`.`SP_CARGAR_PROPECTOMODCERO_DATATABLE`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	}
	...
	$sQuery = "CALL `afiliacion`.`SP_CARGAR_CUENTA_PROSPERCTOMODCERO_DATATABLE`();";
