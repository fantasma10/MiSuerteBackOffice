<?php
	 ...
	 */
	$aColumns = array( 'nIdIncremental', 'sRFC', 'sRazonSocial', 'sNombreComercial', 'nIdEstatus','nIdTipoPersona','nIdPais', '\'\'', '\'\'' );
	$aVisibleColumns = array( $aColumns[1], $aColumns[2] );
	...
	if ( $_GET['sSearch'] != "" ) {
		...
		$sQuery = "CALL `afiliacion`.`SP_CARGAR_PROSPECTOAUTORIZAR_DATATABLE`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	} else {
		$sColumns = str_replace(" , ", " ", implode(", ", $aColumns));
		$sQuery = "CALL `afiliacion`.`SP_CARGAR_PROSPECTOAUTORIZAR_DATATABLE`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	}
	...
	$sQuery = "CALL `afiliacion`.`SP_CARGAR_CUENTA_PROSPECTOAUTORIZAR_DATATABLE`();";
