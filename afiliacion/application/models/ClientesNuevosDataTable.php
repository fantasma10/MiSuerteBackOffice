<?php
	 ...
	 */
	$aColumns = array( 'idCliente', 'RFC', 'RazonSocial', 'idSubcadena', 'idEstatus','FecRegistro','\'\'', '\'\'' );
	$aVisibleColumns = array( $aColumns[1], $aColumns[2], $aColumns[3], $aColumns[4], $aColumns[5] );
	...
	if ( $_GET['sSearch'] != "" ) {
		...
		$sQuery = "CALL `afiliacion`.`SP_CARGAR_CLIENTESNUEVOSCONTABILIDAD_DATATABLE`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	} else {
		$sColumns = str_replace(" , ", " ", implode(", ", $aColumns));
		$sQuery = "CALL `afiliacion`.`SP_CARGAR_CLIENTESNUEVOSCONTABILIDAD_DATATABLE`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	}
	...
	$sQuery = "CALL `afiliacion`.`SP_CARGAR_CUENTA_CLIENTESNUEVOSCONTABILIDAD_DATATABLE`();";
