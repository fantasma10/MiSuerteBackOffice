<?php
	 ...
	 */
	$aColumns = array(  'numero', 'nIdSucursal', 'sNombreSucursal', 'sRFC', 'RazonSocial','nIdEstatus', '\'\'', '\'\'' );
	...
	if ( $_GET['sSearch'] != "" ) {
		...
		$sQuery = "CALL `afiliacion`.`SP_CARGAR_SUCURSALESAUTORIZAR_DATATABLE`( '$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	} else {
		$sColumns = str_replace(" , ", " ", implode(", ", $aColumns));
		$sQuery = "CALL `afiliacion`.`SP_CARGAR_SUCURSALESAUTORIZAR_DATATABLE`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	}
	...
	$sQuery = "CALL `afiliacion`.`SP_CARGAR_CUENTA_SUCURSALAUTORIZAR_DATATABLE`();";
