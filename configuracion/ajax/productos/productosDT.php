...
//echo $idcliente;
	 ...
	 */
	$aColumns = array( 'a.idProducto', 'b.descFamilia', 'c.descSubFamilia', 'a.descProducto', 'a.idEstatusProducto','\'\'','\'\'','\'\'' );
	...
	if ( $_GET['sSearch'] != "" ) {
		...
		$sQuery = "CALL `redefectiva`.`SP_SELECT_PRODUCTOS_DATATABLE`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	} else {
		$sColumns = str_replace(" , ", " ", implode(", ", $aColumns));
		$sQuery = "CALL `redefectiva`.`SP_SELECT_PRODUCTOS_DATATABLE`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	}
$rResult = $WBD->SP( $sQuery );
   ...
   $sQuery = "CALL `redefectiva`.`SP_SELECT_CUENTA_RESULTADOS_DATATABLE`();";
	...
	$sQuery = "CALL `redefectiva`.`SP_SELECT_CUENTA_PRODUCTOS_DATATABLE`('$sLimit', '$sOrder', '$sWhere','{$_GET['sSearch']}');";
