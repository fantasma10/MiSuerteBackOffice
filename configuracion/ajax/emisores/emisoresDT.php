...
//echo $idcliente;
	 ...
	 */
	$aColumns = array( 'idEmisor','descEmisor', 'abrevNomEmivosr', 'b.descEstatus','\'\'','\'\'','\'\'' );
	$aVisibleColumns = array( $aColumns[0], $aColumns[1], $aColumns[2], $aColumns[3], $aColumns[4]);
	...
	if ( $_GET['sSearch'] != "" ) {
		...
		$sQuery = "CALL `redefectiva`.`SP_SELECT_EMISORES_DATATABLE`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	} else {
		$sColumns = str_replace(" , ", " ", implode(", ", $aColumns));
		$sQuery = "CALL `redefectiva`.`SP_SELECT_EMISORES_DATATABLE`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	}
$rResult = $WBD->SP( $sQuery );
  ...
  $sQuery = "CALL `redefectiva`.`SP_SELECT_CUENTA_RESULTADOS_DATATABLE`();";
	...
	$sQuery = "CALL `redefectiva`.`SP_SELECT_CUENTA_EMISORES_DATATABLE`('$sLimit', '$sOrder', '$sWhere','{$_GET['sSearch']}');";
