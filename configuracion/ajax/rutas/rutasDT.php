...
//echo $idcliente;
	...
	$aColumns = array( 'idRuta', 'descRuta', 'b.descProducto', 'c.nombreProveedor', 'd.descConector', 'skuProveedor' ,'\'\'','\'\'','\'\'' );
	$aVisibleColumns = array( $aColumns[0], $aColumns[1], $aColumns[2], $aColumns[3], $aColumns[4], $aColumns[5], $aColumns[6]);
	
	//'idRuta', 'descRuta', 'b.descProducto', 'c.nombreProveedor', 'd.descConector', 'skuProveedor' 
	/* 
	 * Paging
	 */
	$sLimit = "";
	...
	if ( $_GET['sSearch'] != "" ) {
		...
		$sQuery = "CALL `redefectiva`.`SP_SELECT_RUTAS_DATATABLE`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	} else {
		$sColumns = str_replace(" , ", " ", implode(", ", $aColumns));
		$sQuery = "CALL `redefectiva`.`SP_SELECT_RUTAS_DATATABLE`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	}
$rResult = $WBD->SP( $sQuery );
  ...
  $sQuery = "CALL `redefectiva`.`SP_SELECT_CUENTA_RESULTADOS_DATATABLE`();";
	$rResultFilterTotal = $WBD->SP( $sQuery );
	$aResultFilterTotal = mysqli_fetch_array($rResultFilterTotal);
	$iFilteredTotal = $aResultFilterTotal[0];
	/* Total data set length */
	$sQuery = "CALL `redefectiva`.`SP_SELECT_RUTAS_CUENTA_DATATABLE`('$sLimit', '$sOrder', '$sWhere','{$_GET['sSearch']}');";
