...
//echo $idcliente;
	 ...
	 */
	$aColumns = array( 'idProveedor','RFC', 'nombreProveedor', 'razonSocial', 'numCuenta','\'\'','\'\'','\'\'' );
	...
	if ( $_GET['sSearch'] != "" ) {
		...
		$sQuery = "CALL `redefectiva`.`SP_SELECT_PROVEEDORES_DT`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	} else {
		$sColumns = str_replace(" , ", " ", implode(", ", $aColumns));
		$sQuery = "CALL `redefectiva`.`SP_SELECT_PROVEEDORES_DT`('$sLimit', '$sOrder', '$sWhere', '{$_GET['sSearch']}');";
	}
$rResult = $WBD->SP( $sQuery );
  ...
  $sQuery = "CALL `redefectiva`.`SP_SELECT_CUENTA_RESULTADOS_DATATABLE`();";
	...
	$sQuery = "CALL `redefectiva`.`SP_SELECT_CUENTA_PROVEEDORES_DT`('$sLimit', '$sOrder', '$sWhere','{$_GET['sSearch']}');";
