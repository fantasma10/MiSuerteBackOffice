...
function cargapermisos(){
    ...
    var cadenat     = $('#idcadena').val();
    var clientet    = $('#idcliente').val();
    var sucursalt   = $('#cmbSucursal').val();
   /* if(textidprod == ''|| textidprod == -1){textidprod= -1;}
    if(sucursalt == ''|| sucursalt == -1){sucursalt= -1;}
    if(clientet == ''|| clientet == -1){clientet= -1;sucursalt= -1;}
    if(cadenat == ''|| cadenat == -1){cadenat= -1;clientet= -1;sucursalt= -1;}*/
    
    if(textidprod == ''|| textidprod == -1){textidprod= -1;}
    if(sucursalt == ''|| sucursalt == -1){sucursalt= -1;}
    if(clientet == ''|| clientet == -1){clientet= -1;}
    if(cadenat == ''|| cadenat == -1){cadenat= -1;}
   ...
   /* if(textidprod == ''){
        toasts('Debe seleccionar un producto antes de filtrar'); return;
           ...
           $.post('./ajax/permisos/productopermisosesp.php',{prod:textidprod,cadenat:cadenat,clientet:clientet,sucursalt:sucursalt},function(mensaje){
...
function busquedacadena(){
         ...
         $("#txtcadena").autocomplete({
			source: function(request,respond){
				$.post( "./ajax/permisos/cadenasAC.php", { "strBuscar": request.term },
				function( response ) {
					respond(response);
				}, "json" );
			},
			minLength: 1,
			focus: function(event,ui){
				$("#txtcadena").val(ui.item.snombreCadena);
				return false;
			},
			select: function(event,ui){
				$("#idcadena").val(ui.item.nIdCadena);
			...
			search: function(){
               ...
               //$('#txtcliente').attr('disabled',true);	
                $("#cmbSucursal").html('<option value="-1">--</option>');
                $("#cmbSucursal").attr('disabled', true);
			}
		})
		.data( "ui-autocomplete" )._renderItem = function( ul, item ) {
			return $( '<li>' )
...
function busquedcliente(){
    
 // var cadenam  = $("#idcadena").val();
    
    
         $("#txtcliente").autocomplete({
             
			source: function(request,respond){
				$.post( "./ajax/permisos/clientesAC.php", { "strBuscar": request.term,"cadenax":$("#idcadena").val()},
				function( response ) {
					respond(response);
				}, "json" );
			},
			...
			select: function(event,ui){
				$("#idcliente").val(ui.item.nIdCliente);
                $("#idcadena").val(ui.item.nIdCadena);
                $("#txtcadena").val(ui.item.sCadNombre);
				busquedasucursal(ui.item.nIdCliente);
				$("#cmbSucursal").attr('disabled', false);	
...
function busquedasucursal(clientx){
    

    
    $.post('./ajax/permisos/cargasucursal.php',{cliente:clientx},function(res){
        
        $('#cmbSucursal').html(res);
...
function borraidcadena(){
    
    var textnomocad = $('#txtcadena').val();
    
    
    if(textnomocad == '' || textnomocad.length < 4){
...
function borraidcliente(){
    
    var textnomcte = $('#txtcliente').val();
    
    
    if(textnomcte == '' || textnomcte.length < 4){
...
function loadrutas2(productof){
    
   
    
    $.post('./ajax/permisos/rutasporprod.php',{prod:productof},function(mensaje){
...
var cadenap     = $('#idcadena').val();
var clientep    = $('#idcliente').val();
var sucursalp  = $('#cmbSucursal').val();
   /* if(textidprod == ''|| textidprod == -1){textidprod= -1;}
    if(sucursalt == ''|| sucursalt == -1){sucursalt= -1;}
    if(clientet == ''|| clientet == -1){clientet= -1;sucursalt= -1;}
    if(cadenat == ''|| cadenat == -1){cadenat= -1;clientet= -1;sucursalt= -1;}*/
    ...
    if(sucursalp == ''|| sucursalp == -1){sucursalp = -1;}
    if(clientep == ''|| clientep == -1){clientep = -1;}
    if(cadenap == ''|| cadenap == -1){cadenap = -1;}   
    ...
    if(cnf == true){
       ...
       $.post('./ajax/permisos/guardarpermisos.php',{idpermisoo:idpermisoo,version:version,ruta:ruta,producto:producto,cadenap:cadenap,clientep:clientep,sucursalp:sucursalp,maximo:maximo,minimo:minimo,impcomercio:impcomercio,impgrupo:impgrupo,impusuario:impusuario,impespecial:impespecial,impcosto:impcosto,porcomercio:porcomercio,porgrupo:porgrupo,porusuario:porusuario,porespecial:porespecial,porcosto:porcosto,usr:usr,estatus:estatus},function(res){
...
function cargraproducto(idproducto){
    
    $.post('./ajax/permisos/cargaproducto.php',{idproducto:idproducto},function(res){
        
    $("#idproducto").val(res.idprod); 
    $("#txtProducto").val(res.nomprod); 
     
   loadrutas2(idproducto);
...
function loadedicion(idpermiso, idproducto){
    
    cargraproducto(idproducto);
    setTimeout(function(){editapermiso(idpermiso);},2000);
